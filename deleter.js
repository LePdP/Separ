...
async function cleanExcessActions() {
  for (;;) {
    ...
    for (let i = 0; i < btUsers.length; i++) {
      let user = btUsers[i];
      if (user.blockCount >= 3000) {
        logger.info('trimming old actions for', user);
        await sequelize.query('DELETE FROM Actions WHERE typeNum = 1 AND source_uid = ? AND updatedAt < DATE_SUB(NOW(), INTERVAL 10 DAY) LIMIT 10000;',
          {
